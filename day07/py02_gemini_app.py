...
genai.configure(api_key='AIzaSyBHqZkfXkCvL9s-sYJaSZUVfZ2yxzsyFUU')
model = genai.GenerativeModel('gemini-1.5-flash')

class window(Tk): # 오버라이딩: 재정의
    def __init__(self):
        super().__init__() # 부모객체도 같이 초기화
        self.title('제미나이 챗봇 v2.0')
        self.geometry('730x450')
        self.iconbitmap('./image/robot_1.ico')
        self.protocol('WM_DELETE_WINDOW', self.onClosing) # 클래스 작업할 땐 self...유심히
