...
# 6. 제미나이 API용 구성
genai.configure(api_key='AIzaSyBHqZkfXkCvL9s-sYJaSZUVfZ2yxzsyFUU') # 신청한 API키
model = genai.GenerativeModel('gemini-1.5-flash') # AI 모델 (사전 훈련됨)
...
def responseMessage():
    ...
    if inputText:
        ...
        finally: # 에러가 나든 말든 무조건 실행
            textResult.see(END) # 스크롤텍스트 마지막위치로 스크롤 다운

#8. textMessage 위젯에서 엔터를 치면 전송버튼 클릭
def keypress(event):
    # print(repr(event.char)) # repr을 안쓰면 \r,\x80 표시 안 됨
    # \r(캐리지 리턴) \n(뉴라인) 혼용해서 사용 \r\n, \n, \r
    if event.char == '\r':
        responseMessage()
# 종료시 이벤트처리 함수
...
# 8. 입력창에서 엔터시 keypress 이벤트처리 함수발생
...
# 5. API호출 결과메시지 출력될 스크롤기능이 있는 텍스트 위젯
