...
class Block:
    ...
    def move(self): # 볼 무브
        # radians: 각도(dir)를 입력받아 radian으로 변환
        # 볼이 움직이는 x축 값을 계속 게산, x는 dir 값을 라디언으로 변환후 코사인처리
        self.rect.centerx += math.cos(math.radians(self.dir)) * self.speed   
        # 볼이 움직이는 y축 값을 계속 게산, y는 dir 값을 라디언으로 변환후 사인처리
        self.rect.centery -= math.sin(math.radians(self.dir)) * self.speed

    def draw_E(self): # 공을 circle이 아니라 ellipse로 생성 (center계산 복잡해서)
        pygame.draw.ellipse(Surface, self.col, self.rect) # 타원

    def draw_R(self):
        pygame.draw.rect(Surface, self.col, self.rect) # 사각형
